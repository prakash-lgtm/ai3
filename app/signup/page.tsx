...
import { UserPlus, Mail, Lock, User, ArrowRight, Loader2, Zap } from "lucide-react";
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        // If we got HTML, don't show the whole thing, just a generic error
      ...
      >
        <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[32px] p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-purple-600 to-blue-600 flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(147,51,234,0.3)]">
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div className="space-y-1.5 px-1">
              ...
              <input
                ...
                className="w-full bg-white/[0.04] border border-white/10 rounded-2xl py-3 px-4 text-white placeholder:text-gray-600 outline-none focus:border-purple-500/50 focus:bg-white/[0.08] transition-all"
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 disabled:opacity-50 text-white font-bold py-4 rounded-2xl shadow-[0_10px_20px_-10px_rgba(147,51,234,0.4)] transition-all flex items-center justify-center gap-2 group relative overflow-hidden"
              >
                {loading ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
                  <>
                    <span className="relative z-10">Initialize New Account</span>
        ...
        <p className="mt-8 text-center text-gray-600 text-[10px] uppercase tracking-[0.2em] font-medium">
          Identity Secured by Quantum Shield • v2.0
        </p>
      </motion.div>
    </div>
  );
